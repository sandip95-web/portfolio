...
const PortFolioPage = () => {
  ...
  const x = useTransform(scrollYProgress, [0, 1], ['0%', '-80%']); // Adjusted useTransform
  ...
  return (
    ...
    >
      <div className="h-[600vh] relative" ref={ref}>
        <div className="w-screen h-[calc(100vh-6rem)] flex items-center justify-center text-8xl text">
          My Works
        </div>
        <div className='sticky top-0 flex h-screen items-center gap-4 overflow-hidden'>
          ...
          {items.map((item)=>(
            <div className={`w-screen h-screen flex items-center justify-center bg-gradient-to-r ${item.color}`} key={item.id}>
              <div className='flex flex-col gap-8 text-white'>
                ...
                <Link href={item.link} className='flex '><button className="p-2 text-sm md:p-4 md:text-md lg:p-8 lg:text-lg bg-white text-gray-600 font-semibold m-4 rounded">View Code</button></Link> 
              </div>
            </div>
          ))}
          </motion.div>
